...
export class Sort {
  ...
  create_buffer() {
    ...
    if (!pmmgSettings["PMMGExtended"]["sorting"])	// Initialize the stored sorting settings if they don't exist
    {
    ...
    pmmgSettings["PMMGExtended"]["sorting"].forEach(settings => {	// For each stored sorting option, test to only show the ones corresponding to the current planet
      ...
      modifyColumn.appendChild(createSmallButton("edit", createAddInterface, [sortObj, tile, pmmgSettings, parameters, settings]));	// Create the edit button
      modifyColumn.appendChild(createSmallButton("delete", function (pmmgSettings, row, settings) {	// Create the delete button

        for (var i = 0; i < pmmgSettings["PMMGExtended"]["sorting"].length; i++)	// For each stored setting, find the one corresponding to the current row
        {
  ...
  saveButton.addEventListener("click", function () {
    ...
    if (prefilled) {
      for (var i = 0; i < pmmgSettings["PMMGExtended"]["sorting"].length; i++) {
        if (pmmgSettings["PMMGExtended"]["sorting"][i] == settings) {
          pmmgSettings["PMMGExtended"]["sorting"][i] = [itemAbbreviation, parameters[1], sortingInfo, getCheckOfPopupRow(burnRow), getCheckOfPopupRow(zeroRow)];
          break;
        }
      }
    } else {
      pmmgSettings["PMMGExtended"]["sorting"].push([itemAbbreviation, parameters[1], sortingInfo, getCheckOfPopupRow(burnRow), getCheckOfPopupRow(zeroRow)]);
    }
    setSettings(pmmgSettings);
