...
async function createSummaryScreen(tile, parentBuffer) {
  var storageValue = await getLocalStoragePromise("PMMG-Action");
  var storedActions = storageValue["PMMG-Action"] || {};
  ...
  importButton.addEventListener("click", function () {
    ...
    popup.addPopupRow("dropdown", "Type", ["Paste JSON", "Upload JSON", 0], "Which style of import to use. Ping Pi314 on Discord for format specifics. Official documentation coming soon.", function () {
  ...
  packageNames.forEach(name => {
    ...
    deleteButton.addEventListener("click", function () {
      showWarningDialog(tile, "Are you sure you want to delete this action package?", "Confirm", async function () {
  ...
  */
  ...
  private generateGroupForm() {
    ...
    this.groups.forEach((group, groupIndex) => {
      ...
      switch (group.type)	// Populate content column based on material group type
      {
        case "Manual":
          if (group.materials) {
            var contentText = "";
      ...
      deleteButton.addEventListener("click", function () {
        showWarningDialog(thisObj.tile, "Are you sure you want to delete this group?", "Confirm", function () {
  ...
  private createGroupPopup(groupIndex) {
    ...
    switch (group.type) {
      ...
      case "Manual":
        ...
        if (group.materials) {
          Object.keys(group.materials).forEach(mat => {
            popup.addPopupRow("text", "Material Ticker #" + (numMaterials + 1).toLocaleString(), mat, undefined, undefined);
            popup.addPopupRow("number", "Material Amount #" + (numMaterials + 1).toLocaleString(), group.materials[mat], undefined, undefined);
            numMaterials++;
          });
        }

        // Row for adding more material rows above
        popup.addPopupRow("button", "Add Material", "ADD", "Add a new material to the group.", function () {

          const newMatRow = popup.addPopupRow("text", "Material Ticker #" + (numMaterials + 1).toLocaleString(), undefined, undefined, undefined);
          const newAmtRow = popup.addPopupRow("number", "Material Amount #" + (numMaterials + 1).toLocaleString(), undefined, undefined, undefined);
          numMaterials++;
    ...
    popup.addPopupRow("button", "CMD", "SAVE", undefined, function () {
      ...
      if (name && name != "") {
        ...
        popup.destroy();
        thisObj.generateGroupForm();
      } else	// If no valid name, throw an error
      {
        nameRow.row.classList.add(...Style.FormError);
      }
    });
  ...
  private generateActionForm() {
    ...
    this.actions.forEach((action, actionIndex) => {
      ...
      */
      ...
      // Add delete button functionality
      deleteButton.addEventListener("click", function () {
        showWarningDialog(thisObj.tile, "Are you sure you want to delete this action?", "Confirm", function () {
    ...
    // Dropdown for adding new groups
    const newActionDropdown = this.createFormRow(this.actionSection, "select", "active", "Action Type", "actionType", undefined, ["CX Buy"]);
  ...
  private createActionPopup(actionIndex) {
    ...
    switch (action.type) {
      case "CX Buy":
        ...
        popup.addPopupRow("button", "Price Limits", "EDIT", undefined, function () {
          ...
          if (action.priceLimits) {
            Object.keys(action.priceLimits).forEach(mat => {
              pricePopup.addPopupRow("text", "Material Ticker #" + (numMaterials + 1).toLocaleString(), mat, undefined, undefined);
              pricePopup.addPopupRow("number", "Price Limit #" + (numMaterials + 1).toLocaleString(), action.priceLimits[mat], undefined, undefined);
              numMaterials++;
            });
          }

          // Row for adding more material rows above
          pricePopup.addPopupRow("button", "Add Material", "ADD", "Add a new material to enter a price limit for.", function () {

            const newMatRow = pricePopup.addPopupRow("text", "Material Ticker #" + (numMaterials + 1).toLocaleString(), undefined, undefined, undefined);
            const newAmtRow = pricePopup.addPopupRow("number", "Price Limit #" + (numMaterials + 1).toLocaleString(), undefined, undefined, undefined);
            numMaterials++;

            // Move rows above save rows
            pricePopup.form.insertBefore(newMatRow.row, pricePopup.form.children[pricePopup.form.children.length - 4]);
            pricePopup.form.insertBefore(newAmtRow.row, pricePopup.form.children[pricePopup.form.children.length - 3]);
          });
        ...
        popup.addPopupRow("checkbox", "Buy Partial", action.buyPartial, "Whether the action will be taken if there is not enough stock on the CX.", undefined);

        // Add row for using CX inventory
        popup.addPopupRow("checkbox", "Use CX Inventory", action.useCXInv == undefined ? true : action.useCXInv, "Whether to use stock in the CX warehouse when calculating how much needs to be bought.", undefined);
        break;
    }
  ...
  private generateSaveForm() {
    ...
    saveButton.addEventListener("click", async function () {
      ...
      if (thisObj.globalAttributes.name)	// Remove old action package under previous name to prevent proliferation with name changes
      {
...
function executeAction(actionPackage, executionIndex, messageBox, executionInfo, tile, skipButton, cancelButton, currentAction) {
