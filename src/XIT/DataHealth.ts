...
export class DataHealth {
  ...
  create_buffer() {
    ...
    if (!this.userInfo["PMMG-User-Info"])	// This should almost never happen.
    {
      this.tile.appendChild(createTextSpan("No collected data is present. Refresh the buffer. If the problem persists, contact Pi314 on Discord."));
      return;
    }
    ...
    const cxPriceAge = this.userInfo["PMMG-User-Info"]["cx_prices"] ? ((Date.now() - this.userInfo["PMMG-User-Info"]["cx_prices"]["Age"]) / 3600000).toLocaleString(undefined, { maximumFractionDigits: 0 }) + "h" : false as any;
