    ...
    case "Repair":
      ...
      if (site.buildings) {
        site.buildings.forEach(building => {
          if (!NonProductionBuildings.includes(building.buildingTicker) && Date.now() - building.lastRepair > parseFloat(info.days) * 86400000)	// Old enough
          {
...
function updateStoredChecklists(result, params) {
  ...
  if (isChild)	// If a child, look at the children of each element
  {
  ...
  } else	// If not a child, just look at the top level
  {
...
class CheckItem	// Individual checklist item. Held within a checklist.
{
  ...
  constructor(checkInfo, checklist, isChild?) {
    ...
    if (!this.isChild)	// Children can't be edited
    {
      ...
      getLocalStorage("PMMG-Checklists", function (result) {	// First, check whether the object is still completed (in local storage)
        ...
        if (completed) {
          ...
          if (thisObject.checkInfo.isChild) {
            ...
            toDelete = false;
          } else if (thisObject.checkInfo.recurring && thisObject.checkInfo.duedate)	// Recur
          {
          ...
          // Make the update
          getLocalStorage("PMMG-Checklists", updateStoredChecklists, [thisObject.checkInfo, thisObject.checklist, toDelete, thisObject.isChild]);	// Delete or recur the object
        }
      });
    }, 2500);
  }
