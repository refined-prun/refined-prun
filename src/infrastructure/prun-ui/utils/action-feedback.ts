...
export async function waitActionFeedback(
  ...
  const timedOut = Symbol('Action feedback timeout');
  const timeoutResult: WaitActionFeedbackResult = {
    result: 'error',
  ...
  const timeout = new Promise<typeof timedOut>(resolve => {
    if (options.timeoutMs !== undefined) {
      timeoutId = setTimeout(() => resolve(timedOut), options.timeoutMs);
    }
  });
  try {
    const closed = waitNodeDisconnected(tileFrame);
    const overlay = await Promise.race([$(tileFrame, C.ActionFeedback.overlay), closed, timeout]);
    if (overlay === timedOut) {
    ...
    if (overlay.classList.contains(C.ActionConfirmationOverlay.container)) {
      ...
      if ((await Promise.race([waitNodeDisconnected(overlay), timeout])) === timedOut) {
        return timeoutResult;
      }
