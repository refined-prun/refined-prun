  ...
  run(allBuffers) {
    ...
    calcTags.forEach(tag => {
      ...
      buffers.forEach(buffer => {
        if (
          ((buffer.children[3] || buffer.children[2]).firstChild as HTMLElement).classList.contains(this.tag) ||
          ((buffer.children[3] || buffer.children[2]).children[1] as HTMLElement).classList.contains(this.tag)
        ) {
          return;
        }
        ...
        (buffer.children[3] || buffer.children[2]).insertBefore(
          calcDiv,
          ((buffer.children[3] || buffer.children[2]).firstChild as HTMLElement).id == 'refresh'
            ? (buffer.children[3] || buffer.children[2]).children[1]
