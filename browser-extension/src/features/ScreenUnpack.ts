...
import { WithStyles, Style } from '@src/Style';
import { genericCleanup, createNode } from '@src/util';
...
export class ScreenUnpack implements Module {
  ...
  getScreensAndAttachDropdownMenuListeners() {
    ...
    (Array.from(menu.children) as HTMLElement[]).forEach(dropdownMenuElement => {
  ...
  updateHighlight(screenName: string) {
    ...
    for (let i = 0; i < buttons.length; i++) {
      ...
      if (button.children[0].textContent?.toUpperCase() == screenName) {
        ...
        button.children[1].classList.add(...WithStyles(Style.ScreenUnderlineToggled));
      } else {
        button.children[1].className = '';
        button.children[1].classList.add(...WithStyles(Style.ScreenUnderlineUntoggled));
      }
    }
  }
}
