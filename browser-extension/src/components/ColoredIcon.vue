...
if (props.ticker === 'SHPT') {
  ...
  name = getMaterialNameByTicker(props.ticker) ?? '???';
  category = materialCategory.value?.name ?? 'unknown';
}
...
<template>
  <div :class="PrunCss.ColoredIcon.container" :style="style" :title="name" @click="onClick">
