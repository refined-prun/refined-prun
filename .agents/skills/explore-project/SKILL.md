...
**UX Note:** Ask the user to confirm the planned action before execution. The user can decline, after which you select the next priority.
...
Map the project's directory structure. List all directories (not individual files) under the project root, excluding common non-source directories: `node_modules`, `.git`, `dist`, `build`, `.next`, `target`, `__pycache__`, `.gradle`, `bin`, `obj`, `.agents`, `.claude`, `docs`, `vendor`, `.venv`, `env`. Limit depth to 3-4 levels.
...
Found: **N stale**, **N gaps**, **N shallow**
