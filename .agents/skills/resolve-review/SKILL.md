---
name: resolve-review
description: Apply resolutions from a PR review file under .tmp/pr. The user writes resolution notes first, then this skill applies each fix, commits it separately, and removes resolved findings. Use for "resolve review", "fix review", "apply review", or "resolve findings". Do not use without an existing review file; run $review-pr first.
---
...
The commit message must describe the code change, not the review finding. Do not add model attribution or a co-author trailer unless the user requests it. Examples:
