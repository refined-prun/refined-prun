---
name: augment-review
description: Extract human review comments from the working copy and append them as findings to pr-review.md. Validates that all changes are comment-only, then removes only the captured additions. Use for "augment review", "add review comments", "human review", or "add findings". Do not use without an existing review file; run $review-pr first.
---

# Augment PR Review with Human Comments

Read the working copy diff, extract every added comment as a structured finding, append to `pr-review.md`, and remove only the captured comment additions.
...
## Phase 2.5: Remove the captured comments

Validation passed. Reverse only the saved comment additions so the user can continue working. Do not reset unrelated files.

```bash
git apply --reverse .tmp/pr/<number>/augment-diff.txt
```

If reversal fails, stop and keep the saved diff. Do not use a broad checkout or reset command.
...
>
> The captured comment additions were removed in Phase 2.5. Run `$augment-review` again to add more, or write resolution notes and run `$resolve-review`.
