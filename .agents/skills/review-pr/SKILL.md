---
name: review-pr
description: Review a GitHub pull request against project guidelines and create a structured review under .tmp/pr. Accepts a PR number. Use for "review pr", "review pull request", "pr review", or "check pr". Do not use for code review outside a pull request.
---
...
**Principle:** Report everything. This workflow can check out the PR branch, merge its base branch, run Prettier, and commit formatting. Get explicit user approval before these mutations.
...
```bash
.agents/skills/review-pr/scripts/preflight.sh
```
...
The PR number must contain digits only. Stop if the resolved value does not match this rule.

Before running either setup command, tell the user exactly which branch operations and possible formatting commit will occur. Ask for explicit approval. If the user does not approve, stop without changing the repository.

**If the current branch's PR number matches the target PR number:**

```bash
.agents/skills/review-pr/scripts/setup-pr.sh <number>
```

**Otherwise (need to checkout):**

```bash
.agents/skills/review-pr/scripts/setup-pr.sh <number> --checkout
```

**If the script exits non-zero:** It prints `ERROR: MERGE_CONFLICT`. Stop. Tell the user that the PR branch conflicts with its base branch and needs manual resolution.
...
The approval from Phase 2 covers this formatting pass and its possible `prettier` commit. If the user approved setup but excluded commits, skip the formatting script and record Prettier as not run.
...
```bash
.agents/skills/review-pr/scripts/format-and-lint.sh <number>
```

```bash
.agents/skills/review-pr/scripts/gather-context.sh <number>
```
...
```bash
.agents/skills/review-pr/scripts/worktree.sh create <number>
```
...
```bash
.agents/skills/review-pr/scripts/worktree.sh remove <number>
```
...
The `.agents/skills/review-pr/scripts/worktree.sh create` script auto-cleans stale worktrees before creating. If it still fails, check `git worktree list` for conflicts and remove the exact conflicting entry.

### Stale worktree from interrupted review

Phase 5.5 cleans up automatically. To clean up manually: `.agents/skills/review-pr/scripts/worktree.sh remove <number>`
