...
**UX Note:** Present the restructuring plan and ask the user to approve it before making changes.
...
Read every `.md` file in `docs/` (excluding `exploration-manifest.md`). Do NOT read or modify `AGENTS.md` or `AGENTS.md` — those are managed separately. For each file, build a section map:
...
- Search all doc files, `AGENTS.md`, and `AGENTS.md` for cross-references (links like `docs/old-name.md` or references like "see architecture.md") and update them to point to the new locations
