...
- If it belongs in `AGENTS.md` → edit it in.
- If it belongs in a `docs/` file → edit it in.
- If it is too minor for docs, add a pending item to the current working plan so the user can decide.

**Open questions:**
- Add a pending item to the current working plan with the text `Investigate: <question>`.
